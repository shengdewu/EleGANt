...
from .modules.pseudo_gt import fine_align, expand_area, mask_blur, fine_align_no_face
...
def generate_pgt_no_face(image_s, image_r, mask_s, mask_r, lms_s, lms_r, margins, blend_alphas, img_size=None):
    """
    input_data: (3, h, w)
    mask: (c, h, w), lip, skin, left eye, right eye
    """
    ...
    # tps align
    pgt = fine_align_no_face(img_size, lms_r, lms_s, image_r, pgt, mask_r, mask_s, margins, blend_alphas)
    return pgt
...
class ComposePGT(nn.Module):
    def __init__(self, margins, skin_alpha, eye_alpha, lip_alpha, no_face=False):
    ...
    def forward(self, sources, targets, mask_srcs, mask_tars, lms_srcs, lms_tars):
        ...
        for source, target, mask_src, mask_tar, lms_src, lms_tar in \
            ...
            if self.no_face:
                pgt = generate_pgt_no_face(source, target, mask_src, mask_tar, lms_src, lms_tar,
