...
def detect_np(image: np.ndarray) -> 'faces':
    h, w = image.shape[:2]
...
def crop_img_np(image: np.ndarray, face, up_ratio=0.6 / 0.85, down_ratio=0.2 / 0.85, width_ratio=0.2 / 0.85) -> (np.ndarray, 'face'):
...
def landmarks_np(image: np.ndarray, face):
    shape = predictor(image, face).parts()
